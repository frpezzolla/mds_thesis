...
cep_df['abortion'] = cep_df['abortion'].replace({r"^\s*<NA>\s*$": pd.NA}, regex=True).astype("string")
cep_df = cep_df.dropna(subset=['abortion'])
...
for batch_start in tqdm(range(0, N_SIMS, BATCH_SIZE), desc='simulating'):
    ...
    batch_contexts = [
        ...
        "Responde a la siguiente pregunta, delimitada por los símbolos <>, de acuerdo con las alternativas de respuesta delimitadas por corchetes []." for _, sample in batch_samples.iterrows()
    ]

    alternatives = [
        "A: El aborto debe estar siempre prohibido",
        "B: El aborto solo debe estar permitido en casos",
        "C: El aborto debe ser una libre opcion para las mujeres",
        "D: Ninguna"
    ]
    ...
    batch_messages = [
        [
            ...
            {"role": "user", "content": (
                "Pregunta: <¿Cuál de estas ideas expresa de mejor modo su juicio respecto del aborto?> "
                f"Alternativas: [{alt_text}]. "
                'Responde en formato JSON con las siguientes llaves: "razonamiento" (máximo 100 palabras), donde explicas tu razonamiento, y "respuesta", usando solo la letra (A, B, C o D) correspondiente a la alternativa elegida.'
            )}
